...
- gets list of given security prices
'''
...
get_security_prices_url = "https://api.canopy.cloud:443/api/v1/securities/prices.json"
...
#add/remove which securities to include
tickers = ["GE_US", "AAPL_US", "NESN_VX"]

querystring = {"tickers[]":tickers,"from_date":from_date,"to_date":to_date}
headers = {
    'authorization': token,
    'username': username,
    'content-type': "application/x-www-form-urlencoded; charset=UTF-8",
...
response = requests.request("GET", get_security_prices_url, headers=headers, params=querystring)
