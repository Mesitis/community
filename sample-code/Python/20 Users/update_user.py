...
- Forgets password - sending recovery info to provided email (if user exists on email)
'''
...
update_user_url = "https://api.canopy.cloud:443/api/v1/users/{{user_id}}.json"
...
#replace below with a user ID
user_id = "1111"

update_user_url = "https://api.canopy.cloud:443/api/v1/users/" + user_id + ".json"

#replace below with display name and email
display_name = ""
email = ""
...
response = requests.request("POST", update_user_url, data=payload, headers=headers)

print json.dumps(response.json(), indent=4, sort_keys = True)

#may thrown internal server error due to a blank/incorrct user id in the url
