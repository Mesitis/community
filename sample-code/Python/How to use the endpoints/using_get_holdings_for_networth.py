...
- if the user is a regular customer then get a list of holdings for this user
- if the user is a partner_admin then get a list of holdings for the first user from the list of users this partner admin has access to
'''
...
get_holdings_url = "https://api.canopy.cloud:443/api/v1/portfolio/holdings.json"
...
#replace date to get holdings from any given date, to calculate the networth of that date
date = "28-07-2017"
querystring = {"date":date,"date_type":"traded_on","output_format":"json","group_by":"custodian"}
headers = {
    'authorization': token,
    'username': username,
    'content-type': "application/x-www-form-urlencoded; charset=UTF-8",
...
response = requests.request("GET", get_holdings_url, headers=headers, params=querystring)

# This function creates a dictionary with the contents of the json data for holdings
holdings = json.loads(json.dumps(response.json()))
Total = 0

# Loop iterates through all of the holdings summing up their USD value
for counter in range(0,len(holdings)):
        Total = Total + float(holdings[counter]["market_val_rpt_ccy"]) 
print Total
