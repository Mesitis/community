...
- Returns signature for uploading documents
'''
...
get_upload_signature_url = "https://api.canopy.cloud:443/api/v1/uploads/signature.json"
...
#in case the user is  partner_admin then switch_user_id is any one of the users it has access to (here we take the first one from the list)

#fill in file details below
file_name = ""
file_size = ""
file_type = ""

querystring = {"name":file_name,"size":file_size,"type":file_type}
...
response = requests.request("GET", get_upload_signature_url, headers=headers, params=querystring)
