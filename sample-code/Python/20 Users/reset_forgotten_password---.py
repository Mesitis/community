...
- Resets the forgotten password with new password
'''
...
reset_forgotten_password_url = "https://api.canopy.cloud:443/api/v1/users/password/forget/reset.json"
...
#replace below with forgotten password token and new password
forgotten_password_token = ""
new_password = ""

payload = {"token": forgotten_password_token, "password" : new_password}
headers = {
    'authorization': token,
    'username': username,
    'content-type': "application/x-www-form-urlencoded",
...
response = requests.request("POST", reset_forgotten_password_url, data=payload, headers=headers)
